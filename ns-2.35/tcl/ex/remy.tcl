...
set opt(seed) 0
set opt(simtime) 400.0;        # total simulated time
...
set opt(app) FTP
set opt(onoffavg) 20.0;              # mean on and off time
...
#            puts [$recvapp($i) results]
            ...
            puts "Connection $i: $totalbytes bytes $throughput bits/s avgsrtt $avgrtt"
        }
    }
...
for {set i 0} {$i < $numsrc} {incr i} {
    set ranvar($i) [new RandomVariable/Exponential]
    $ranvar($i) set avg_ $opt(onoffavg)
}
...
#        puts "$curtime($i): Turning on $i"
    ...
    while {$curtime($i) < $opt(simtime)} {
        set nexttime [expr $curtime($i) + [$ranvar($i) value]]
